...
__device__
void multibrot(float c_x, float c_y, int area, int max_iter, float d,
    ...
    // Non-black colour if not in multibrot set
    if (iter != max_iter) {
        // Smoothened value for number of iterations
        float iter_smooth = iter - logf(logf(z_abs)) / logf(d) + 1;
...
// Kernel that runs multibrot on subpixel and averages RGB values.
// This subsampling results in an anti-aliasing effect.
__global__
void multibrot_aa(int size_x, int size_y, int grid_size, int max_iter, float d,
                  float x_bounds[2], float y_bounds[2], float gradient, char *colours) {
    ...
    float c_x;
    float c_y;
    // Running multibrot in each subpixel
    for (int grid_x = 0; grid_x < grid_size; ++grid_x) {
        for (int grid_y = 0; grid_y < grid_size; ++grid_y) {
            ...
            multibrot(c_x, c_y, area, max_iter, d, R, G, B, gradient);
        }
    }
...
int main() {
    ...
    for (int n = 0; n < 250; n ++) {
        ...
        std::cout << std::endl;
        multibrot_aa<<<dimGrid, dimBlock>>>(size_x, size_y, grid_size, max_iter, d, x_bounds, y_bounds, gradient, colours);
        ...
        image.write(std::format("images/multibrot-{}.png", n));
