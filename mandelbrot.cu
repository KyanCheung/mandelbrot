...
__device__
void mandelbrot(float c_x, float c_y, int area, int max_iter,
                float& R, float& G, float& B, float gradient) {

    // Check if c is in cardioid
    float q = (c_x - .25) * (c_x - .25) + c_y * c_y;
    ...
    if (iter != max_iter) {
        ...
        float H = fmodf(iter_smooth * sqrt(iter_smooth) * gradient, 360.0f);
...
int main() {
    ...
    float x_bounds[2] = {-3.2, 1.6};
    float y_bounds[2] = {1.35, -1.35};
    float x_centre = -0.8212;
    float y_centre = -0.2006;
    ...
    for (int n = 0; n < 320; n ++) {
        ...
        std::cout << "x-bounds: " << x_bounds[0] << ", " << x_bounds[1] << std::endl;
        std::cout << "y-bounds: " << y_bounds[1] << ", " << y_bounds[0] << std::endl;
        std::cout << std::endl;
